...
| Command | Description |
|---------|-------------|
| `init` | Set up AI agent config files (`.claude`, `.cursor`, `.windsurf`) in the output directory |
| `agent-help` | Print a usage guide for AI coding agents to read |
| `clean` | Remove old log sessions. Use `--retain N` to keep the last N sessions (default: 5) |
...
| `--follow, -f` | Follow logs in real-time as new entries arrive (default: true) |
| `--output, -o` | Output directory for log files (default: `./logs`) |
| `--since, -s` | Only show logs since a duration ago (e.g., `5m`, `1h`) or RFC3339 timestamp |
...
| Flag | Description |
|------|-------------|
| `--exclude, -e` | Exclude log lines matching this regex pattern. Example: `--exclude 'health.*check'` |
| `--mute, -m` | Hide matching containers from terminal output (still written to log files). Example: `--mute 'worker'` |
